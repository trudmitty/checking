...
широкие команды, в которых закодированы операции для всех исполнительных устройств процессора,
которые запускаются на данном такте. Получается, что в одной широкой команде может быть несколько
операций, выполняющихся паралельно.
...
значений. При вызове функции в архитектурах x86 и ARM используется стек вызовов:
перед переходом в функцию значения регистров, которые должны сохраниться
после возврата, сохраняются в памяти (стеке) с помощью соответствующих инструкций, 
а по завершении функции значения восстанавливаются из стека. В
...
стек связующей информации (или "стек возвратов") (*PCS*) хранят данные о вызовах процедур и
возвратах, а адрес возврата и контекст предыдущей процедуры размещаются
...
адресом, размером и текущим смещением, с указателями
*PSP* и *PCSP*, и тесно интегрированы с регистровым файлом через механизм
аппаратной подкачки и откачки. Пользовательский стек задаётся базовым адресом и размером,
...
архитектура использует аппаратные регистровые окна и несколько стеков. 
При вызове процедуры в регистровой части стека связующей информации запоминается
целый набор информации о текущей функции - указатель команды продолжения функции,
ее контекст, база окна в регистровом файле, состояние предикатного файла, дескриптор стека
пользователя, регистр состояния процессора и др., которая упаковывается в два квадрослова.
Здесь и возникает ключевая проблема для stackful корутин, так как в классической реализации корутин
предполагается, что для переключения контекста достаточно подменить стек и несколько регистров,
но на Эльбрусе недостаточно сменить стек без согласованного изменения состояния процедуры. Архитектура
Эльбрус ориентирована на строго контролируемую модель вызова процедур. В результате возобновление
выполнения корутины может происходить в некорректном контексте, с неправильными регистрами и данными.
В результате можем рассматривать только stackless корутины, где состояние сохраняется отдельно от стека.
...
[здесь](http://www.mcst.ru/files/511cea/886487/1a8f40/000000/book_elbrus.pdf).
